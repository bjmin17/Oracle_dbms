...
CREATE TABLE tbl_student(
    ...
    st_dep_no	nVARCHAR2(3)	NOT NULL	
);
...
-- 학번이 2번부터 4번까지인 학생의 리스트를 이름 순으로 보고 싶다.
SELECT * FROM tbl_student
WHERE st_num BETWEEN 'A0002' AND 'A0004'
ORDER BY st_name;

-- 학번이 2번부터 4번까지인 학생의 리스트를 학년의 역순으로 보고 싶다.
SELECT * FROM tbl_student
WHERE st_num BETWEEN 'A0002' AND 'A0004'
ORDER BY st_grade DESC;

-- tbl_student의 테이블에 저장된 데이터 레코드가 모두 몇 개냐?
SELECT COUNT(*)
FROM tbl_student;

-- 학생테이블에서 2학년 학생들의 데이터 레코드가 모두 몇 개냐?
...
-- 이미 존재하는 레코드의 특정칼럼에 값을 입력하고자 할 때는
-- UPDATE 명령을 수행한다.
...
-- 결국 이름, 학년, 학과 까지 WHERE 조건에 걸어서 UPDATE 수행하려고 하는데
