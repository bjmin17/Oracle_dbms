...
-- 이 조인이 표시하는 리스트를 카티션 곱 이라고 표현한다.
SELECT * 
...
-- 두 테이블간에 참조무결성을 무시하고 JOIN 시행하기
-- 새로운 도서가 입고가 되었는데 그 동안에 사용하던 장르와 완전 다른 분야이다.
-- 그래서 새로운 장르 코드를 생성해서 010이라고 사용하기로 결정을 했다.
...
FROM tbl_books B, tbl_genre G
WHERE B.b_genre = g.g_code ;
-- 그 결과 EQ JOIN으로 확인해보니 새로 등록한 도서리스트가 누락되어 출력됐다.
...
SET b_genre = '장르소설'
WHERE b_genre = '장편소설';
-- 이 UPDATE 명령문을 실행하면 최소 2개 이상의 레코드가 변경(UPDATE) 될 것이다.
-- 데이터베이스에서 1개 테이블에 2개 이상의 레코드가 변경되는 실행은
