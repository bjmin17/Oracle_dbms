...
MySQL : limit 라는 속성으로 매우 편리하게 pagination을 구현할 수 있다.
mssql : offset 과 limit 라는 속성
기타 DBMS : limit, offset과 같은 개념들이 있어서 편하게 구현이 가능하다.

오라클은 limit, offset 등을 실제적으로 지원하지 않는다.
오라클 자은 개발자, DBA가 limit 등을 수행하지 않아도
...
하지만 오라클에서는 order by와 where 절을 같이 사용하지만 않으면
...
데이터가 많을 때
순서가 앞에 있는 레코드를 먼저 찾는 옵티마이저 알고리즘을 작동시켜라

*/

/* 
from에 테이블 대신 다른 SELECT 쿼리를 사용한 SQL 작성
from절에 사용한 SELECT를 inline view라고 한다.

FIRST_ROWS
where 절에서 ROWNUM의 가상 칼럼 값을 N 이하로 설정하면
...
(   -- 1. 데이터를 불러오고
    SELECT /*+ INDEX_DESC(P) FIRST_ROWS */ * FROM tbl_product P -- inline view
) IP
-- 2. 10개씩 잘라서
WHERE ROWNUM <= 10;

-- fetch : 실제 물리 데이터(file, dbf)를 불러와서 cursor(보고자하는 데이터)로 보냄
-- cursor 를 이용해서 페이지네이션 구현

-- BETWEEN 사용할때는 힌트가 작동하지 않는다.
...
FIRST_ROWS
9i 이하에서는 cost base로 optimizer 실행
10 이상에서는 index base로 optimizer 실행을 해서 table에 index가 없거나 레코드의 개수가 현저히 적으면 오히려
옵티마이저 하지 않은 것보다 늦게 나오는 경우가 있다.

그래서, FIRST_ROW(정수) optimizer를 제한하거나
10 이상에서는 FIRST_ROWS_10, FIRST_ROWS 100, FIRST_ROWS 1000 이라는 형식으로 작성해야
좀 더 효율적으로 작동이 된다.
