...
이름       널?       유형             
-------- -------- -------------- 
ST_NUM            VARCHAR2(3)    복제하면 PRIMARY KEY 조건이 삭제됨
ST_NAME  NOT NULL NVARCHAR2(50)  -- NOT NULL, UNIQUE를 제외한 상당 부분이 삭제
ST_TEL            VARCHAR2(20)   
ST_ADDR           NVARCHAR2(125) 
ST_GRADE          NUMBER(1)      
ST_DEPT           VARCHAR2(3)  
...
-- 테스트를 위해서
-- tbl_student 테이블을 tbl_test_std 테이블로 통째 복제를 했는데
-- 테이블을 복제하면, 데이터 형식, 데이터들 그리고 NOT NULL 등 일부는 그대로 복제가 되는데
-- PK 등 중요한 제약조건들은 무시되고 복제되지 않는다.
-- 복제된 테이블에 제약조건 추가를 해야 한다.

-- 또는 
-- 테이블을 생성할 당시에는 제약조건들이 필요하지 않아서
-- 작성하지 않았는데 나중에 사용하다 보니 제약 조건들이 필요한 경우가 생겨서
-- 테이블을 새로 작성하지 않고 제약조건들만 추가하는 방법

-- 1. NOT NULL 제약조건
--  칼럼에 값이 없으면 INSERT가 실행되지 않도록 하는 제약조건
ALTER TABLE tbl_test_std MODIFY(st_name nVARCHAR2(3) NOT NULL);
...
-- TABLE CREATE 할 때는 단순히 PRIMARY KEY만 지정해주면 PK 칼럼을 설정할 수 있었는데
-- ALTER TABLE을 사용해서 PK를 추가할 때는 가급적 이름을 지정해 주는 것이 좋다.
-- DBMS에 따라서 이름을 지정하지 않으면 실행이 안 되는 경우도 있다.
...
-- 이름이 지정되지 않으면 나중에 조건이 필요없어서 삭제를 하고자 할 때
-- 삭제가 어려워질 수도 있다.
