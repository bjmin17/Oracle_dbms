...
-- 학생정보를 저장할 tbl-student Table 생성

-- tbl_student 라는 이름으로 table(물리적 저장소)를 생성한다.
-- tbl_student : 생성할 table 이름
-- 이름명명규칙 : Java에서 변수, 클래스, method 등의 이름을 명명하는 것과 같다
-- 단, 오라클에서는 대소문자를 구별하지 않는다.
-- 테이블 명명 패턴 : 
--      일반적으로 테이블을 만들 때 테이블 이름앞에 접두사로 tbl_ 사용한다.
-- 
CREATE TABLE tbl_student(
    -- 칼럼 : 필드(멤버)변수와 같은 개념
    -- 칼럼들은 컴마(,)로 구분하여 나열
    -- 칼럼들의 data type 지정을 한다.
    ...
    --      숫자로 인식을 해버리는 오류가 있다.
    -- 그래서 오라클에서는 아주 특별한 경우가 아니면 그냥 VARCHAR2()로 사용한다.
    st_num CHAR(5),
    
    -- VARCHAR2() : 가변길이 문자열 저장 칼럼
    -- 최대 4000글자까지 저장
    -- 최대 글자까지 저장하는 것은 CHAR와 유사하지만
    ...
    st_age NUMBER(3) -- 제일 마지막 칼럼은 뒤에 컴마를 찍지 않는다.
);
...
-- 이후에 tbl-student 테이블에서 0100의 학생데이터를 조회 했더니
-- 데이터가2개가 조회되었다.
-- 이런 상황이 되면 이 0100 학생의 데이터 2개 중에 어떤 데이터가 
-- 이 학생의 데이터인지 알기가 매우 어려워진다.
-- 여러가지 데이터를 다시 검증해야만 어떤 데이터가 정상적(사용할 수 있는)인
--      데이터인지 확인 할 수 있다.

-- 이런 문제가 발생하지 않도록 미리 무언가 조치를 취해 줘야 하는데
-- DB 에서는 이런 문제를 발생하지 않도록 설정을 할 수 있다.
...
VALUES('00001', '성춘향', '010-1111-1111');
...
-- 오라클이 보내는 메시지 중에 "unique constraint"가 포함되면
...
-- 그랬더니
-- 오라클이 : ORA-01400: cannot insert NULL into ("USER1"."TBL_STUDENT"."ST_NAME")
...
-- tbl-student 테이블은 학생정보를 보관하는 매우 중요한 table이다.
-- tbl-student에서 어떤 학생의 데이터를 조회하고자 할 때는
