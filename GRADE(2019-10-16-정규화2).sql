...
학생정보 칼럼을 제2정규화
score 테이블에는 학생 이름이 일반적인 문자열로 저장이 되었다
일반적인 문자열로 저장된 경우
...
여러 개의 레코드를 수정, 변경하는 것을 지양하도록 한다.

또한, 만약 학생 정보에 다른 정보를 포함하고 싶을 때는
...
학생정보 테이블을 생성하고
학생 코드를 만들어 준 다음
score 테이블의 s_std 칼럼의 값을 학생 코드로 변경하여
제2정규화를 수행하고자 한다.
...
--      tbl_student의 학번 정보를 tbl_score에 등록
SELECT COUNT(*)
FROM tbl_score SC, tbl_student ST
    WHERE  SC.s_std = ST.st_name; -- 600개 나오는지 확인,, 누락이 있으면 안됨
...
UPDATE tbl_score SC
SET SC.s_stcode = 
(SELECT ST.st_num FROM tbl_student ST WHERE ST.st_name = SC.s_std); -- 600개 행 업데이트

SELECT SC.s_stcode, ST.st_num, SC.s_std, ST.st_name
FROM tbl_score SC, tbl_student ST
    WHERE SC.s_stcode = ST.st_num; -- 검증
