...
/*
 ...
 객체 무결성
 1. 내가 어떤 데이터를 조회했을 때 나타나는 데이터는 '내가 필요로 했던 데이터이다' 라는 보장
 2. PK를 WHERE 조건으로 SELECT를 했을 때 나타나는 데이터는 1개의 레코드이며 
    이 데이터는 '내가 원하는 데이터이다' 라는 보장
    
 3. PK는 1개의 칼럼을 지정하는 것이 원칙이지만
    실제 상황에서 1개의 칼럼만으로 PK를 지정하지 못하는 경우도 있다.
    이럴 때는 2개 이상의 복수 칼럼을 묶어서 PK로 지정하는 경우도 있다.
    
    예) 거래처정보
    거래명 + 대표 + 전화번호 칼럼을 묶어서 PK로 지정하는 경우
    
 4.복수의 칼럼을 PK로 지정하는 경우
    UPDATE,DELETE를 수행할 때 PK를 WHERE 조건으로 명령을 수행할 때
 ...
 6. '코드' 칼럼은 최초 데이터를 추가하기 위해 준비단계에서
    ...
    'ID' 칼럼은 실제 데이터와 관계없이 '일련번호', 'SN(Serial Number)' 형식으로
    지정하는 경우도 많다.
    ...
    하지만
    오라클 11이하에서는 AUTO INCREMENT라는 옵션이 없어서
    여러가지 방법으로 사용한다.
    
    그 중 가장 많이 사용하는 방법이
    SEQUENCE Object를 생성하고 SEQUENCE의 NEXTVAL 값을 활용하여
    데이터를 추가할 때 ID 칼럼에 새로운 값이 만들어져 저장되도록
    사용한다.
...
GUID
    ...
    범 우주적으로 유일한 값 (암호화한 키로도 사용했었음)
    32Byte의 중복되지 않는 키값을 생성
    
    60E70E31E51544CA8D3A4F2C8097E63E 
    
    오라클에서는 
    GUID를 저장할 칼럼의 데이터 형식을 
    RAW(무한히 크기가 제한이 없는 바이너리 형태) 값으로 지정하거나
    nVARCHAR2(125) 이상으로 지정해서 사용을 한다.
*/

SELECT SYS_GUID() FROM DUAL; --60E70E31E51544CA8D3A4F2C8097E63E
...
-- DBMS 업무 중에 b_name(도서명)과 b_writer(저자)를 조건으로 하는
...
하지만 개발초기에 많은 양의 데이터를 INSERT를 수행해야 할 경우에는
INDEX를 일단 설정하지 말고 사용하는 것이 효율적이다.

초기 데이터를 추가할 때 가급적이면
PK로 설정된 칼럼을 기준으로 정렬된 원본 데이터로 INSERT를 수행하는 것이 효율적이다.
