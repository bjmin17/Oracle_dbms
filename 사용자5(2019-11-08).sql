...
/*
    ...
    가장 좋은 방법은 1개의 유일한 값을 갖는 칼럼을 선정하여 PK 설정
...
-- 1. 전체칼럼에 모든 데이터를 다 입력하고 데이터를 추가할 때만 가능
-- 2. table의 칼럼 순서대로 데이터를 나열해야 한다.

-- 만약 업무(프로젝트)가 진행되는 과정에서 
-- table의 칼럼 순서가 구조가 변경되는 경우
-- 칼럼을 추가하는 하는 경우 보통 table구조에서 끝부분에 칼럼이 추가되는데
-- 이 때는 데이터가 순서대로 원하는 칼럼에 저장될 것이다 라는 보장을 할 수 없게 된다.

SELECT * FROM tbl_memo;

-- m_seq 칼럼에 값이 1인 데이터를 추가된 상태에서
-- 다시 m_seq 칼럼의 값이 1인 데이터를 또 추가하려고 하면
-- PK 규칙에 어긋나서 데이터가 추가 되지 않는다.
-- 이런 상황에서 새로운 데이터를 추가할 때마다
-- m_seq 값을 기존값과 중복되지 않은 값을 찾아야 하는데
-- 이 방법이 업무진행에서 매우 번거로운 작업
INSERT INTO tbl_memo (m_seq, m_date, m_time, m_auth, m_subject)
VALUES(1,'2019-11-08','09:42:00','홍길동','메모작성');
...
-- SELECT, INSERT, UPDATE 명령 분 내에서 사용이 되면
...
SELECT 
    ...
    SUM(DECODE(io_inout,'매출',io_total,0)) AS 매출
FROM tbl_iolist
GROUP BY SUBSTR(io_date,0,7); --2018-06	719177	5591000, 2018-10	2949488	1908700
...
SELECT 
    ...
    SUM(DECODE(io_inout,'매출',io_total,0)) AS 매출
FROM tbl_iolist
GROUP BY io_dname; -- 서원상사	0	257800, (주)동양엔지니어링(광주남구롯데)	0	312200
...
SELECT 
    ...
    SUM(DECODE(io_inout,'매출',io_total,0)) AS 매출
FROM tbl_iolist
GROUP BY io_dname,io_dceo; --두영산업	김웅겸	58253	245000, 롯데가스보일러(수성롯데)	호세진	34932	439000

-- 거래명세와 거래처정보 table을 JOIN하여 확인하기
-- EQ(inner) JOIN은 JOIN하는 테이블이 서로 참조무결성이 보장되는 경우 가능
-- 참조무결성이 위배되는 테이블을 EQ JOIN을 실행하면
-- 누락된 데이터가 존재하는지 알 수 있다.
SELECT *
FROM tbl_iolist, tbl_dept
WHERE io_dname = d_name AND io_dceo = d_ceo;
--353	2018-04-20	해바라기쵸코볼	서원상사	김종석	매출	79	700	55300	D0091	서원상사	김종석	010-5246-6054	
--354	2018-04-20	핫쵸코	(주)동양엔지니어링(광주남구롯데)	김기국	매출	62	1500	93000	D0120	(주)동양엔지니어링(광주남구롯데)	김기국	010-4074-8392	

-- 참조무결성이 설정되지 않은 테이블 간의 JOIN
