...
-- 표준 SQL에서는 PK 지정방식을 칼럼에 PRIMARY KEY 키워드 지정 방식으로 사용하는데
-- 간혹 표준 SQL의 PK 지정방식이 안 되는 DBMS가 있다.
-- 이 때는 별도의 CONSTRAINT 추가 방식으로 지정해야 한다.
...
GROUP BY s_subject
ORDER BY s_subject; -- 과학 국사 국어 미술 수학 영어
...
/*
    ...
    하지만, 사용 중에 과목이 추가되거나 과목명이 변경되는 경우
    테이블의 칼럼을 변경해야 하는 상황이 발생한다.
        테이블의 칼럼을 변경하는 것은 DBMS 입장에서나 사용자 입장에서
        많은 비용(사용중지 시간, 노력, 위험성 등)을 지불해야 한다.
...
FROM tbl_score
WHERE s_std = '갈한수'
GROUP BY s_std -- GROUP BY로 묶고 DECODE를 집계함수로 묶어줌
ORDER BY s_std;
...
FROM tbl_score
GROUP BY s_std
--ORDER BY s_std; -- 학생이름 과학 국사 국어 미술 수학 영어 총점 평균 석차
);
