...
FROM tbl_iolist IO, tbl_product P
    WHERE IO.io_pname = P.p_name
        AND IO.io_inout = 1 
GROUP BY io_inout, io_pname, io_price; -- 상품이름이 같은데 단가가 틀린 게 있냐고 묻는것

-- 매입매출 테이블에서 매입단가를 조회해봤더니
-- 다행히 상품이름이 같은데 단가가 다른 상품이 없는 것 같다.
...
SET p_iprice
= ( 
    SELECT MAX(IO.io_price ) -- 높은가격 1개만 추출해서 매입단가로 뽑아냄
            FROM tbl_iolist IO
    WHERE io_inout = 1 AND  -- 일부 쿼리가 오류가 날것, p_name이 2개짜리 개수가 2개인 것도 있기 때문에
...
= ( 
    SELECT MAX(IO.io_price ) -- 높은가격 1개만 추출해서 매입단가로 뽑아냄
            FROM tbl_iolist IO
    WHERE io_inout = 2 AND  -- 일부 쿼리가 오류가 날것, p_name이 2개짜리 개수가 2개인 것도 있기 때문에
        P.p_name = IO.io_pname
    );
...
-- NULL인 값이 있다.
-- 어떤 상품은 매입만, 어떤 상품은 매출만 된 경우

/*
    매입단가에서 매출단가 생성하기
    공산품일 경우 매입단가의 약 6% 를 더해서 매출단가를 계산
