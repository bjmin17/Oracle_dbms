...
UPDATE tbl_score
SET s_kor = ROUND( DBMS_RANDOM.VALUE(50,100),0 ), -- 50~100까지 난수를 만들어서 소수0번째 자리까지
...
SELECT s_num, s_kor, s_eng, s_math,
        ...
        ROUND((s_kor + s_eng + s_math) / 3, 1) AS 평균
FROM tbl_score
-- 계산한 결과 평균이 70 ~ 80 인 결과만 표시
WHERE (s_kor + s_eng + s_math) / 3 BETWEEN 70 AND 80;

-- 통계, 집계함수
-- SUM(), COUNT(), AVG(), MAX(), MIN()

-- 전체리스트의 각 과목별 총점을 계산하고 싶다.
SELECT SUM(s_kor)
FROM tbl_score;
...
FROM tbl_score; -- 237, 191

SELECT s_kor, s_eng, s_math,
        (s_kor + s_eng + s_math) AS 총점
FROM tbl_score; 

-- 개인 총점이 200점 이상인 값 중에서 리스트들의 집계
SELECT SUM(s_kor) AS 국어총점,
        SUM(s_eng) AS 영어총점,
        SUM(s_math) AS 수학총점,
        SUM(s_kor + s_eng + s_math) AS 전체총점,
        ROUND(AVG((s_kor + s_eng + s_math)/3), 0) AS 전체평균
FROM tbl_score
WHERE s_kor + s_eng + s_math >= 200; -- 496, 381, 461, 1338, 74
