-- 여기는 매입매출관리 화면입니다.

-- 매입매출장 TABLE 생성

CREATE TABLE tbl_iolist (

    io_seq	NUMBER	NOT NULL	PRIMARY KEY,
    io_date	VARCHAR2(10)	NOT NULL,	
    io_pname	nVARCHAR2(50)	NOT NULL,	
    io_dname	nVARCHAR2(50)	NOT NULL,	
    io_dceo	nVARCHAR2(20),		
    io_inout	NUMBER(1)	NOT NULL,
    io_qty	NUMBER	NOT NULL,	
    io_price	NUMBER,		
    io_amt	NUMBER		
    
);

SELECT* FROM tbl_iolist; -- 매입 매출을 1,2로 변경하면서 정규화를 시행했음
SELECT io_inout, COUNT(*) FROM tbl_iolist
GROUP BY io_inout; -- 1은 232개, 2는 259개

-- DECODE(칼럼, 값, T값, F값)
SELECT DECODE(io_inout, 1, '매입', '매출') AS 구분,
    COUNT(*) 
FROM tbl_iolist
GROUP BY DECODE(io_inout, 1, '매입', '매출'); -- 매출 259, 매입 232

-- DECODE(칼럼, 값1, T값1, 값2, T값2, 값3, T값3)
SELECT DECODE(io_inout, 1, '매입', 2, '매출') AS 구분,
    COUNT(*) 
FROM tbl_iolist
GROUP BY DECODE(io_inout, 1, '매입', 2, '매출'); -- 매출 259, 매입 232


/*
IF(칼럼 == 값) {
    T표시
} else if (칼럼 == 값2) {
    T표시2
}

1개의 칼럼이 여러가지 값을 가지고 있을 때
다중 조건으로 처리하고자 하면
DECODE는 좋은 코딩 모양을 이룰 수 있다.
DECODE(칼럼, 값1, T결과1, DECODE(칼럼, 값2, T결과2, DECODE(칼럼, 값3, T결과3)))
DECODE(칼럼, 값1, T결과1, 값2, T결과2, 값3, T결과3)
*/


SELECT DECODE(io_inout, 1, '매입', 
        DECODE(io_intout,2 ,'매출'))
FROM tbl_iolist ;

SELECT DECODE(io_inout,1,'매입',2,'매출')
FROM tbl_iolist;

SELECT DECODE(io_inout,1,'매입',2,'매출') AS 거래구분,
        SUM(DECODE(io_inout, 1, io_amt, 0)) AS 매입합계,
        SUM(DECODE(io_inout, 2, io_amt, 0)) AS 매출합계
FROM tbl_iolist
GROUP BY DECODE(io_inout,1,'매입',2,'매출'); --매출 0 41683800, 매입 22688667 0

SELECT  SUM(DECODE(io_inout, 1, io_amt, 0)) AS 매입합계,
        SUM(DECODE(io_inout, 2, io_amt, 0)) AS 매출합계
FROM tbl_iolist; -- 22688667, 41683800 매입과 매출이 피벗방식으로 표시가 됨

-- 날짜별 매입매출 합계
SELECT  io_date, SUM(DECODE(io_inout, 1, io_amt, 0)) AS 매입합계,
        SUM(DECODE(io_inout, 2, io_amt, 0)) AS 매출합계
FROM tbl_iolist
GROUP BY io_date; -- 

-- LEFT : 표준 SQL에서 왼쪽부터 문자열 자르기
-- RIGHT : 오른쪽부터 문자열 자르기
-- MID(문자열,시작,개수) : 중간문자열 자르기

SELECT SUBSTR(io_date, 0, 7) AS 월,
    SUM(DECODE(io_inout, 1, io_amt, 0)) AS 매입합계,
    SUM(DECODE(io_inout, 2, io_amt, 0)) AS 매출합계,
    
    SUM(DECODE(io_inout, 2, io_amt, 0)) -
    SUM(DECODE(io_inout, 1, io_amt, 0)) AS 마진
    
FROM tbl_iolist
GROUP BY SUBSTR(io_date, 0, 7) -- 2018-06 매입합계 매출합계 마진, 2018-10 매입합계 매출합계 마진,,,,
ORDER BY SUBSTR(io_date, 0, 7) ; -- 1월부터 월별로 매입매출합계와 마진을 보여줌 