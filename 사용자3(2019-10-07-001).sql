...
--      삭제 : DROP
--      변경(수정) : ALTER
-- 상황이 이미 생성된 tble의 한 칼럼(b_price)의 자릿수를 늘리려고 한다.
...
-- 테이블을 처음 생성할 당시에미처 생각지 못한 칼럼이 필요한 경우가 있다.
...
-- 저장된 데이터가 변형되어 문제가 발생할 수 있다.
-- 오라클은 명령 수행전에 절대!!! 경고하지 않는다.

-- 2. MODIFY
-- 칼럼의 타입을 변경하는 것으로 저장된 데이터가 변형될 수 있다.
-- 가. 자릿수를 줄이면 : 보통 실행 오류가 발생한다.
--      그렇지 않은 경우도 있는데 이 때는 저장된 데이터의 일부가 잘릴 수 있다.
-- 나. 타입변경
--      기존 데이터 형식이 변경되면서 데이터가 손실, 소실 될 수 있다.
...
--INSERT INTO tbl_books(b-isbn,b_title,b_comp,b_writer,b_price,b_genre)
...
-- 일부 데이터에 빈칸 등이 잘못 삽입되어 데이터를 조회할 때
-- 문제가 발생하는 것을 알게 되었다.
-- 특히, 장르별로 데이터를 조회했더니
...
VALUES('979-007','자바의 정석','도울출판','남궁성','004'); -- 윗 SELECT 실행해도 조회 안됨,, 장르테이블에 코드 004가 없음
