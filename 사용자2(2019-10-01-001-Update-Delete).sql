...
-- 취소하는 명령
ROLLBACK; -- 26번부터 46번까지의 INSERT와 UPDATE를 취소
SELECT * FROM tbl_address;
...
-- 이 상황에서 홍길동 데이터 2개 중에 1개는 주소를 부산광역시로
--      또, 1개는 광주광역시로 바꾸고 싶을 때
--      실행할 수 있는 방법이 없다.
-- 결론적으로 이 table은 설계상 miss로 인해
-- '개체무결성'이 훼손되었다 라고 할 수 있다.
-- '개체무결성'을 보장하기 위해서는 PK를 생성해야 하는데
-- 또한, 현재 상황에서는 어려움이 있다.
-- '개체무결성'을 보장하는 table을 재설계하자.
-- 지금 구상한 주소록 테이블에는 저장된 값들은 중복되지 않는 칼럼을 찾을 수가 없다.
-- 이럴 때는 별도로 PK로 사용할 칼럼을 추가해서 '개체무결성'을 보장하는 방법이 있다.
...
-- 확인이 됐다.
-- 한 개의 홍길동 데이터는 그대로 두고
-- 한 개는 주소를 광주광역시로, 나머지 한 개는 주소를 부산광역시로 바꾸려 한다.
...
-- DELETE 명령도 UPDATE 와 마찬가지로 기본형으로 실행을 절대 하지 말기!!!!!!!
...
-- DELETE를 실행할 때도 table에 PK 가 있으면 반드시 PK 단위로 데이터를 삭제하자
-- 1. 삭제하고 하는 데이터 있는지 여러 방법으로 조회를 해본다.
...
DELETE FROM tbl_address WHERE name = '홍길동'; -- 이라고 실행하면 절대 안된다~!!!

SELECT * FROM tbl_address WHERE name = '홍길동' AND address = '서울특별시' ;
DELETE FROM tbl_address WHERE name = '홍길동' AND address = '서울특별시' ;-- 라고 해도 되지만!!!
...
-- DBMS를 운영하는 과정에서
-- 만에 하나 재난(실수로 수행한 명령으로 데이터 변경, 삭제, 천재지변)이 발생했을 때
-- 데이터를 복구할 수 있는 준비를 해야한다.
-- 1. 백업 : 업무가 종료된 후 데이터를 다른 저장소, 저장매체에 복사하여 보관하는 것
-- 1-1. 복구 : 백업해둔 데이터를 사용 중인 시스템에 다시 설치하여 사용할 수 있도록 하는 것
--      복구는 원상태로 만드는 데 상당한 시간이 필요하고
...
--      저널링 복구
-- 3. 이중화, 삼중화
--      실제 운영중인 운영체제, DBMS, storage 등을 똑같은 구조로
