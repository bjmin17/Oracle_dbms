...
-- 학번은 테이블에 저장이 되어 있는데 학번에 해당하는 학생이 누구인지를 모르겠다.
...
-- SELECT를 수행할 때 보고자하는 칼럼들을 나열하는 것이
-- 효율성 면에서 좋다.
...
-- 성적정보만 추가를 한 경우
-- EQ JOIN을 실행하면 새로운 학생정보의 성점은 출력이 되지 않는다.
SELECT SC.s_num, ST.st_name, SC.s_kor, SC.s_eng, SC.s_math
FROM tbl_score SC, tbl_student ST
WHERE SC.s_num = ST.st_num;
...
-- 참조무결성이 완전하지 않은 경우는 EQ JOIN으로 데이터를 조회하면 안 된다.
-- 이 경우에는 실제 저장되어 있는 데이터들이 누락되어 출력 되기 때문에
-- LEFT JOIN을 사용해서 조회
...
-- 테이블을 임시로 변경
-- VARCHAR2(3) : 영문기준으로 3글자 입력되지만, 한글 3글자 입력 안됨
...
-- 위 SQL을 활용하여 컴퓨터공학 학생들 성적만 확인하고 싶다.
...
-- SELECT SQL을 수행해서 결과를 얻고자 할 때
-- 매우 복잡한 SQL문을 매번 실행하는 것은 매우 비효율적(코딩적 문제, 코딩하는사람에게)이다.
...
-- 단, VIEW 만들기 위한 SQL 문에서는 ORDER BY를 사용할 수 없다.
...
SELECT s_num || ' : ' || st_name
FROM view_score; -- 001 : 갈한수

-- Java 등 프로그래밍 코딩으로 SQL을 작성할 때는
-- LIKE 키워드의 문자열을 연결문자열로 작성해야 한다.
SELECT * FROM view_score
WHERE d_name LIKE '컴퓨터' || '%'; -- 컴퓨터% 조회,, 자바에서 이렇게 작성해야 작동함
...
FROM view_score -- 001 갈한수 ~ 
ORDER BY s_dept; -- 컴퓨터공학 3개 , 영어영문 여러개 나오는게 한개씩만 나옴

-- 다수의 칼럼을 DISTINCT로 묶으면
-- 원하는 결과가 나오지 않는다.
SELECT DISTINCT s_dept, d_name, s_num
FROM view_score
ORDER BY s_dept; -- 001 컴퓨터공학 001, 001 컴퓨터공학 004 ,,,

-- 표준 SQL을 사용할 때
SELECT s_dept, d_name
FROM view_score
GROUP BY s_dept, d_name
ORDER BY s_dept; -- 001 컴퓨터공학, 002 영어영문 ,003 경영학 ,,,

-- GROUP BY
-- 특정칼럼을 기준으로 하여 집계를 할 때 사용하는 명령절
...
ORDER BY s_dept; -- d_name과 d_pro도 조회하고 싶으면 GROUP BY에 반드시 d_name과 d_pro 추가해야함
--001 컴퓨터공학 홍길동 281, 002 영어영문 성춘향 297,,,

-- GROUP BY로 묶어서 부분합을 보고자 할 때
-- 기준으로 하는 칼럼 이외에 SELECT문에 나열된 칼럼들 중
-- 집계함수로 감싸지 않은 칼럼들은
-- GROUP BY 절 다음에 나열을 해주어야 한다.

-- 학번, 학과코드로 묶어서 보여주는 데이터는
-- 의미 없는 명령문이다.
-- ※ GROUP으로 묶어서 집계를 낼 때는
--  어떤 칼럼들을 묶을 것인지에 대한 많은 고민을 해야 한다.
...
GROUP BY s_dept, d_name, d_pro
ORDER BY s_dept; -- 001 컴퓨터공학 홍길동 281 308 285, 002 영어영문 성춘향 297 297 316
