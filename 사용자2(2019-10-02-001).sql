...
-- tbl_address 테이블의 age칼럼에 어떤 값이라도 입력되어 있으면
-- 그 리스트를 보여달라
SELECT * FROM tbl_address WHERE age IS NOT NULL;

-- 데이터 많이(다량으로)추가를 한 후
-- 혹시 중요한 칼럼의 데이터를 누락시키지 않았나를 판별하고자 할 때
-- 사용하는 조회 명령문

-- 오라클에서는 '' (중간에 빈칸 등 아무런 문자열이 없는 형태)는 NULL과 같은 의미로 본다.
...
SELECT * FROM tbl_address ;
-- 레코드 리스트를 확인했더니 CHAIN 칼럼의 값들이 알 수 없는 기호(숫자값)로 저장되어 있다.
-- 현재는 001, 002, 003의 기호값들이 무엇을 의미하는지 알 수가 없다.
-- INSERT를 수행한 사람에게 물어봤더니
-- 001은 가족, 002는 친구, 003은 이웃 이라고 한다.
-- SELECT를 실행할 때 chain이 001이면 가족, 002이면 친구, 003이면 이웃이라는 문자열로
-- 나타났으면 좋겠다.

SELECT id, name, address,chain,
-- chain 칼럼이 값이 001 이면 '가족'이라고 보이고, 아닐 경우
