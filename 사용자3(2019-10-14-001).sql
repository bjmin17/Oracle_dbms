...
SELECT * FROM DUAL; -- X
SELECT '1' AS num, '홍길동' AS name, '컴공과' AS dept FROM DUAL; -- 1 홍길동 컴공과
...
SELECT '1' AS num, '홍길동' AS name, '컴공과' AS dept FROM tbl_student; -- 1 홍길동 컴공과가 쭈욱 나열
...
UNION ALL SELECT '6' AS num, '장영실' AS name, '컴공과' AS dept FROM DUAL -- 1 홍길동 컴공과, 2 이몽룡 컴공과
UNION ALL SELECT '7' AS num, '장길산' AS name, '컴공과' AS dept FROM DUAL;  --,,, 7개 데이터 나열

-- 실제 사용하는 DBMS에 물리적인 테이블을 생성하지 않고
-- 가상의 데이터를 만들어서 사용하고자 할 때 임시로 TABLE 구조와 데이터를
-- 생성하여 테스트 용도로 사용하는 명령 군
-- 이 명령 군을 사용할 때 UNION 키워드를 사용한다.

-- 예전에 이런 식으로 보고서 작성하기도
SELECT * FROM tbl_score ;
SELECT '===' AS 학번, '====' AS 국어,'====' AS 영어,'====' AS 수학 FROM DUAL -- ================================
UNION ALL SELECT '학번' AS 학번, '국어' AS 국어,'영어' AS 영어,'수학' AS 수학 FROM DUAL -- 학번 국어 영어 수학
...
UNION ALL SELECT s_num AS 학번, to_char(s_kor,'999') AS 국어, 
                    ...
                    to_char(s_math,'999') AS 수학 FROM tbl_score -- 내용 표시
UNION ALL SELECT '-----' AS 학번, '----' AS 국어,'----' AS 영어,'----' AS 수학 FROM DUAL -- ------- ---- ---- ----
UNION ALL SELECT '총점' AS 학번, to_char(SUM(s_kor),'99,999') AS 국어, 
                    to_char(SUM(s_eng),'99,999') AS 영어, 
                    to_char(SUM(s_math),'99,999') AS 수학 FROM tbl_score -- 총점 7,370 7,415 7,547
UNION ALL SELECT '===' AS 학번, '====' AS 국어,'====' AS 영어,'====' AS 수학 FROM DUAL; -- =========================

-- to_char(값, 형식)
-- 숫자형 자료를 문자열형 자료로 변환시키는 cascading 함수
-- UNION ALL을 쓸 때는 형식을 맞춰주어야 한다.(그래서 숫자를 문자로 변경)
-- 형식
-- 9 : 숫자자릿수를 나타내는 형식, 실제 출력되는 값의 자릿수만큼 개수를 지정해야 한다.
--      to_char(123,'99999') >>   123
-- 0 : 숫자자릿수를 나타내는 형식, 실제 출력되는 값보다 자릿수가 많으면 나머지는 0으로 채움
...
SELECT to_char(SYSDATE, 'YYYY-MM-DD HH:MI:SS') FROM DUAL; -- 2019-10-14 09:44:28
...
-- 임시로 사용할 테이블 생성
-- WITH AS
-- SQL 테스트 등을 위해 임시로 테이블을 생성하는 오라클 SQL
WITH tbl_temp AS
(
    ...
    UNION ALL SELECT '5' AS num, '장보고' AS name FROM DUAL -- 1 홍길동, 2 이몽룡, 3 성춘향 ,,
)
...
SELECT * FROM tbl_temp
WHERE num IN ('3', '1','5') -- 3 성춘향
ORDER BY name; -- 3 성춘향, 5 장보고, 1 홍길동
...
/*
    ...
    WHERE IN ('기은성', '남동예', '내세원', '방채호', '배채호') 코드를 생성
...
-- 단순한 JOIN을 이용해서 보여주기 어려운 복잡한 통계, 집계 등을 사용할 때
-- 먼저 SUBQ에서 통계, 집계 등을 계산하고
-- 그 결과와 main table을 JOIN 하고자 할 때
SELECT *
FROM tbl_student ST,
    (SELECT SC.s_num, SC.s_kor + SC.s_eng, SC.s_math FROM tbl_score2 SC) SC_S
...
--      subq는 마치 method 처럼 작동
-- 다. subq에서는 WHERE 절을 실행하여 데이터 추출
-- 라. SUM(s_kor+s_eng+s_math)를 실행하여
-- 마. 결과를 return
-- 바. main에서 st_num, 리턴받은결과 형식으로 표시

SELECT st_num, 
    (   SELECT SUM(s_kor + s_eng + s_math) 
        FROM tbl_score SC-- 22332
...
SELECT s_num, s_kor, s_eng, 
    (   SELECT SUM(s_kor + s_eng + s_math) 
        FROM tbl_score SC -- 22332
        WHERE SC_MAIN.s_num = SC.s_num -- 괄호 안의 이 부분이 제일 마지막에 실행됨
    ) AS 총점
