...
/*
 ...
 일련번호이기 때문에 일단 칼럼을 NUMBER로 설정을해서
 1 ~ 입력 순서대로 번호를 부여하겠다.
 ...
 이럴 경우 프로그래밍 언어에서 데이터를 가져다 사용할 때
 문제를 일으킬 수 있다.
 그래서,
 가격 칼럼에 값이 없이 데이터가 추가되면 자동으로 0을 채우도록 하자
*/
-- TABLE 생성에서 기본키를 항상 고민하자
-- 95년 이전에는 ISBN코드가 없는것도 있다는 점을 고려해야 한다.
CREATE TABLE tbl_books (
    b_code NUMBER PRIMARY KEY, --VARCHAR2(4) 가 아닌 NUMBER로
...
VALUES(2,'오라클','생능','서진수',35000); -- 사용하지 말아야할 SQL문

/*
 데이터를 추가할 때마다 b_code 칼럼의 값을 새로 생성하고 싶다.
    1. Random()을 사용하는 방법
    ...
    2. 일련번호를 순서대로 자동으로 생성하도록 칼럼을 설정하기
        -- 오라클 11 이하에서는 불가능
        -- mysql, mssql 등등 에서는 AUTO INCREMENT라는 옵션을 칼럼에 부여하면 된다.
        -- 오라클 12 이상에서는 가능
*/
...
/*
 ...
 다른 DBMS의 AUTO INCREMENT 기능을 대체하여 사용하는 방법
*/

-- 1부터(START WITH), 1씩 증가(INCREMENT BY 1) 하는 형태로
-- 숫자 값을 생성하는 시퀀스 객체 생성
CREATE SEQUENCE SEQ_BOOKS
START WITH 1 INCREMENT BY 1; 

SELECT SEQ_BOOKS.NEXTVAL FROM DUAL; -- 최초실행 시 1, 그 다음 2, 3, 4, ,,,

-- 오라클에서 가장 많이 사용하는
-- 일련번호 부여방법
INSERT INTO tbl_books(b_code, b_name)
VALUES(SEQ_BOOKS.NEXTVAL, '시퀀스연습'); 
SELECT * FROM tbl_books; -- 6번(앞에 5까지 늘렸기 떄문에)부터 추가되서 쭉 나열
...
/*
 ...
 이제 새로 만든 App에서 데이터를 추가할 때 SEQUENCE를 사용하려고 한다.
 
 1. 기존데이터의 SEQ 칼럼의 최대값이 얼마냐 ghkrdls : 589
 2. 새로운 시퀀스를 생성할 때 START WITH : 600 로 설정
...
ALTER SEQUENCE SEQ_IOLIST INCREMENT BY 600;  -- +600한 결과가 나옴
...
SELECT SEQ_IOLIST.NEXTVAL FROM DUAL; -- 실행할 때마다 1씩 더함
-- 현재 SEQ_IOLIST 값
...
/*
 ...
 이 방식은 ORACLE 이외의 다른 DBMS에서는 상당히 복잡하다.
*/
...
SELECT LPAD(30,10,'*') FROM DUAL; -- ********30

-- RPAD
SELECT RPAD(30,10,'A') FROM DUAL; -- 30AAAAAAAA
SELECT 'B' || LPAD(SEQ_BOOKS.NEXTVAL,4,'0') FROM DUAL; -- B0024
...
UNION ALL SELECT LPAD('중화인민공화국', 20, ' ') FROM DUAL; -- 우리\n 대한민국\n 미연방합중국\n 중화인민공화국

SELECT * FROM tbl_books;
INSERT INTO tbl_books (b_code, b_name)
VALUES('B' || TRIM(TO_CHAR(SEQ_BOOKS.NEXTVAL,'0000')), 'SQL연습');
SELECT * FROM tbl_books; -- B0022 ~ B0027

INSERT INTO tbl_books (b_code, b_name)
VALUES('B' || LPAD(SEQ_BOOKS.NEXTVAL, 4,'0'), 'SQL연습2');

SELECT * FROM tbl_books; -- B0028 ~ B0031
