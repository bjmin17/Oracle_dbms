...
-- TABLE을 JOIN 할 때 TABLE들에 칼럼 이름이 같은 이름이 존재하면
-- 반드시 칼럼이 어떤 TABLE에 있는 칼럼인지 명시를 해주어야
-- 문법적 오류가 발생하지 않는다.
-- 그래서 TABLE을 설계할 때 가급적 접두사를 붙여서 만드는 것이 좋고
-- 그렇더라도 JOIN을 할 때 TABLE alias를 설정하여
...
FROM Table1, Table2
    WHERE dept = num
라는 형식으로 SQL을 작성하면 num 칼럼이 누구의 Table인지 알 수 없어서
문법적 오류가 발생한다
