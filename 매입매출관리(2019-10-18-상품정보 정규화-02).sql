...
-- ※ 1개 이상의(전체) 데이터를 대상으로 UPDATE, DELETE를 수행할 때는
...
--  JOIN 조건등을 사용했다
--  JOIN되는 table들에 JOIN조건으로 사용되는 칼럼의 이름이 같지 않은 경우에 가능한 코드
...
/*
    ...
    이 때는, ROLLBACK을 수행해서 CUD를 취소할 수 있다.
    ...
    어디까지 ROLLBACK을 할 것인지 지정할 수도 있다
