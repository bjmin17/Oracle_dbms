...
-- 1개의 테이블에 정리된 데이터를
-- 다른 테이블로 데이터를 분리하여
-- UPDATE 등을 수행할 때 데이터의 무결성을 보장하기 위한 조치
-- 현재 데이터에서 상품의 이름을 변경하고자 한다.
--UPDATE tbl_iolist
--SET io_pname = ''
--WHERE io-pname = '';
-- 이 UPDATE 명령문은 다수의 레코드를 변경하는 코드가 된다.
-- 이코드가 실행되면서 실제로 변경하지 말아야할 데이터들이 변경될 수도 있고
-- 변경해야할 데이터들이 변경되지 않는 경우도 발생할 수 있다.
-- 상품정보를 별도의 테이블로 분리하고
-- 현재 테이블에는 상품 정보 테이블과 연결하는 칼럼만 두어서
-- 상품정보를 최소한으로 변경을 해도
...
FROM tbl_iolist
--WHERE io_inout = '매입'
GROUP BY io_pname -- 상품정보 이름만 가져오기
ORDER BY io_pname; -- 고드름(블루) 384, 델리어트 2745 ,,,

SELECT io_pname FROM tbl_iolist
GROUP BY io_pname; -- 얼음물, 휴대폰 119, 1000다이제초코,, 상품 이름만
...
SELECT COUNT(*) FROM tbl_product; -- 393
...
-- JOIN을 실행할 때 JOIN되는 테이블들에 같은 이름의 칼럼이 있으면
-- 문법오류가 발생할 수 있다.
-- 이럴때는 TABLE에 ALIAS를 설정하고
-- 칼럼을 ALIAS.COLUMN 형식으로 지정해주어야 한다.
SELECT COUNT(*)
FROM tbl_iolist IO, tbl_dept D
WHERE IO.io_dcode = D.d_code; --"column ambiguously defined" 은 dept에 IO_dcode가 추가되었기 때문에 발생,,
-- io_dcode가 iolist 테이블과 dept 테이블 모두 존재해서
