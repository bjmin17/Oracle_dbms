...
-- 단, 최초에 프로젝트를 올릴 때 .gitignore를 먼저 설정해 두어야 한다.
...
-- 반드시 개수와 순서가 서로 일치해야한다.
-- 개수가 일치하지 않으면 오류가 발생을 하고
-- 순서가 일치하지 않으면 엉뚱한 모양(순서)으로 데이터가 추가될 수 있다.
INSERT INTO tbl_student(st_num, st_name, st_addr, st_tel)
...
-- 4. 리스트를 볼 때 원래의 칼럼 이름 대신에 별명을 사용하기
-- 표준 SQL은 AS 별명 형식으로 사용해야 하며, 오라클에서는 AS 를 생략해도 된다.
...
-- 데이터 리스트(row, record) 중에 필요한 부분만 보고 싶을 때
...
-- 많은 양의 데이터를 조회할 때는 속도면에서 다소 유리하다.
-- 조회를 한 후, 특정 칼럼의 값을 응용프로그램에서 사용하려고 할 때
--      위치(index)로 칼럼 값을 추출했을 때 정확히 원하는 위치 값을 보장하여
--      오류를 줄일 수 있다
SELECT * FROM tbl_student;
...
-- OR 조건 : 여러 조건 중에 한가지라도 true인 리스트를 보여라
SELECT * FROM tbl_student
...
-- 학생테이블에서 이름이 '이몽룡' 이거나 주소가 '남원시' 인 데이터들을 보고 싶다.
SELECT * FROM tbl_student
WHERE st_name = '이몽룡' OR st_addr = '남원시' ;

-- 칼럼 값들을 서로 연결해서 한 개의 문자열처럼 리스트를 보는 방법
...
--          어떤 데이터는 '서울시'라고 했다.
-- 이럴 경우 서울특별시 라고 조회를 하면 서울시 인 데이터는 보이지 않고
-- 서울시 라고 조회를 하면 서울특별시 데이터는 보이지 않게 된다.
-- 이럴 때 '서울' 이라는 문자열이 포함된 데이터만 보고 싶을 때
-- 부분 문자열 조건 조회라는 방법을 사용한다.
-- 키워드 : LIKE
SELECT * FROM tbl_student
WHERE st_addr = '서울시' ; -- 출력이 안됨

-- 학생테이블의 주소 칼럼에 저장된 문자열이 '서울'이라는 문자열로 시작되는 데이터들을 보여라
SELECT * FROM tbl_student
WHERE st_addr LIKE '서울%' ;

-- 학생테이블의 주소 칼럼에 저장된 문자열이 '시'라는 문자열로 끝나는 데이터들을 보여라
SELECT * FROM tbl_student
...
SELECT * FROM tbl_student
WHERE st_name LIKE '%길%'; -- 이름 문자열 중간에 '길'이 포함된 모든 데이터
-- LIKE 연산자키워드
--      SELECT 조회명령이 실행될 때 데이터가 많으면 속도가 매우 느려진다.
--      INDEX 라든가 기법들을 사용하여 SELECT 속도를 높이려고 만든 것들이
-- 무력화 되어 버린다.
...
SELECT * FROM tbl_student
WHERE st_tel >= '010-111-0000' AND st_tel <= '010-333-9999' ;

-- 학생데이터를 조회하고 싶은데
-- 이름은 알지 못하고, 학번이 3번부터 6번 사이에 있었던 것 같다.
-- 이럴 때, 학번 데이터는 문자열 이지만 저장된 구조가 5자리로 일정하므로
-- 비교연산자를 사용하여 데이터를 조회해 볼 수 있다.
SELECT * FROM tbl_student
WHERE st_num >= 'A0003' AND st_num <= 'A0006' ;

-- 어떤 범위 내에 있는 데이터 리스트를 보고자 할 때
-- 학번이 A0003 부터 A0006까지 범위의 데이터를 보고자 할 때
SELECT * FROM tbl_student
WHERE st_num BETWEEN 'A0003' AND 'A0006' ;

SELECT * FROM tbl_student;

-- 주소가 익산시, 남원시, 해남군인 모든 데이터를 보고자 할 때
SELECT * FROM tbl_student
...
-- 절대 2개 이상 출력되지 않는다.
-- PK는 절대 중복데이터가 없다.
-- PK는 절대 비어있으면(NULL값) 안 된다. : NULL 값이면 안된다.
...
-- DBMS의 임시 저장소에 임시로 저장된 데이터를 storage에 저장하는 명령
