...
-- 마이그레이션
-- 버전업, 업그레이드
-- 하위버전에서 사용하던 데이터베이스(물리적 storage에 저장된 데이터들)
-- 상위버전 또는 다른회사의 DBMS에서 사용할 수 있도록 변환, 변경, 이전 하는 것들

-- 오라클 DBMS SW(오라클DB, 오라클)를 이용해서 DB관리 명령어를 연습하기 위해서
-- 연습용 데이터 저장공간을 생성을 한다.
...
-- 실습을 위해서 새로운 사용자를 생성하자,, 유저 아이디 설정 후 비밀번호 설정
...
-- 일부 DDL 명령, DML 명령, 일부 DCL 명령을 사용할 수 있는 권한을 갖는다.
GRANT DBA TO user1;
